...
# that takes a file path as input and returns a Document object. It checks the file extension and uses a mapping to determine the appropriate loader 
# class and arguments. It then creates an instance of the loader class with the given file path and arguments, and calls its load method. The function 
# returns the first document loaded by the loader. If the file extension is not supported, it raises a ValueError with an appropriate error message.
...
#  takes a string parameter source_dir and returns a list of Document objects. It uses the glob module to find all files in the source_dir directory 
# that have specific file extensions defined in the LOADER_MAPPING dictionary. It then calls the load_single_document function on each file path and 
# returns a list of the loaded documents.
